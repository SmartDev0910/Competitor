...
const ZoneModal = ({modalVisible, setModalVisible}) => {
  ...
  const handleZonePress = index => {
    if (selectedZoneItems.includes(index)) {
      setSelectedZoneItems(selectedZoneItems.filter(item => item !== index));
    } else {
      setSelectedZoneItems([...selectedZoneItems, index]);
    }
  };

  const ZoneView = ZonesData.map((item, index) => {
    return (
      <DisciplineItem
        key={index}
        icon={item.icon}
        ...
        selected={selectedZoneItems.includes(index)}
        ...
        <Modal animationType="slide" transparent={true} visible={modalVisible}>
          ...
          <View style={styles.ModalView}>
            <View style={styles.Appbar}>
              <Text style={styles.ModalTitleFont}>Zone</Text>
              <TouchableOpacity
            ...
            <ScrollView>
              <View style={styles.ModalContentView}>{ZoneView}</View>
            </ScrollView>
