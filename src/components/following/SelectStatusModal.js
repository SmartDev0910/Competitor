...
import StatusesData from '../../constants/following/statuses';
import DisciplineItem from './DisciplineItem';

const width = Dimensions.get('window').width;
const height = Dimensions.get('window').height;

const SelectStatusModal = ({modalVisible, setModalVisible}) => {
  ...
  const handleStatusItemPress = index => {
    if (selectedStatusItems.includes(index)) {
      setSelectedStatusItems(
        selectedStatusItems.filter(item => item !== index),
      );
    } else {
      setSelectedStatusItems([...selectedStatusItems, index]);
    }
  };

  const StatusView = StatusesData.map((item, index) => {
    return (
      <DisciplineItem
        key={index}
        icon={item.icon}
        ...
        selected={selectedStatusItems.includes(index)}
        onPress={() => handleStatusItemPress(index)}
      />
    );
  });
      ...
      <SafeAreaView>
        <Modal animationType="slide" transparent={true} visible={modalVisible}>
          ...
          <View style={styles.ModalView}>
            ...
            <ScrollView>
              <View style={styles.ModalContentView}>{StatusView}</View>
            </ScrollView>
