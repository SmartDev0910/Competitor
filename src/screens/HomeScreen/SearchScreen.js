...
import {RecentSearch, SearchedResult} from '../../constants/home/searches';
...
function SearchScreen({navigation}) {
  ...
  const [isSeeAllShow, setIsSeeAllShow] = useState(false);
  const [showCounts, setShowCounts] = useState(10); // To show initial 10 items
  ...
  const handleLoadmore = () => {
    let newCounts = showCounts + 10;
    setShowCounts(newCounts);
  };

  const handleSeeAll = () => {
    setIsSeeAllShow(true);
  };
  ...
  const SearchedResultView = (
    <View style={styles.ContentView}>
      {SearchedResult?.slice(0, showCounts).map((item, index) => {
        return (
          <View style={styles.SearchedResultView} key={index}>
      ...
      {showCounts < SearchedResult.length && (
        <View style={styles.SearchedResultBottom}>
          <Text style={styles.LoadmoreFont} onPress={handleLoadmore}>
            Load more
          </Text>
          <Text style={styles.LoadmoreFont} onPress={handleSeeAll}>
            See all {'>'}
          </Text>
        </View>
      )}
  ...
  const NotFoundView = (
    <View style={styles.ContentView}>
      <View style={styles.SearchedItemRow}>
        <Image source={SearchWeakIcon} style={styles.CancelWeakImage} />
        <Text style={styles.RecentItemFont}>No events found...</Text>
      </View>
    </View>
  );
